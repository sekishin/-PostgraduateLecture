Makefile を用意しているので，make とだけ入力すれば良い．
...
ProcShareTest4.c
  共有メモリの割当まで行ってフォークすることおよび子プロセスを複数生成
  することを除き，ProcShareTest.c と同じ．
  フォーク前の割当が有効であるか，複数の子プロセスでも有効であるかを検
  証．
...
ProdCons.c
  以下の生産者-消費者問題
  ・生産者プロセスでの処理：以下の処理を引数回繰り返し，終了する．
    ・20～80の範囲の数字をランダムに発生する．
    ・発生した数字をリングバッファに入れる．
    ・自分のID，発生した数字，現在バッファに入っている数字の個数を表示する．
    ・20ミリ秒～80ミリ秒の範囲でランダムにスリープする．
  ・消費者プロセスでの処理：以下の処理を引数回繰り返し，終了する．
    ・リングバッファから数字を取り出す．
    ・自分のID，取り出した数字，現在バッファに入っている数字の個数を表示する．
    ・取り出した数字分のミリ秒をスリープする．
  ・プログラムは，リングバッファの大きさと繰返し回数を引数とする．
